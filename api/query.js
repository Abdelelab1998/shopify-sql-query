...
/* -------------------------------------------------------
   DUCKDB WASM BUNDLE (STATIC — REQUIRED FOR VERCEL)
------------------------------------------------------- */
const bundle = {
  mainModule: duckdb.DUCKDB_WASM,
  mainWorker: duckdb.DUCKDB_WORKER
};

/* -------------------------------------------------------
   RUN SQL USING DUCKDB WASM
------------------------------------------------------- */
async function runSQLQuery(sql, tableName, rows) {
  ...
  // Register SHA2 hashing for SQL
  await conn.register_udf("SHA2", value => {
    return CryptoJS.SHA256(String(value)).toString();
  });

  // Execute FULL SQL (SELECT expressions, WHERE, GROUP BY, etc.)
  try {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === "connect") {
      ...
      const encoded = encodeSession({ storeName, accessToken });
      const secure =
      ...
      res.setHeader(
        "Set-Cookie",
        `shopify_session=${encoded}; Path=/; HttpOnly; SameSite=Lax; Max-Age=86400${
          secure ? "; Secure" : ""
        }`
      );
    ...
    if (session) {
      ...
      return res.status(401).json({
        error: "Not connected. Please connect your store first."
      });
    }

    /* -------------------------------------------------------
       DETECT TABLE NAME IN SQL
    ------------------------------------------------------- */
    const match = sql.toLowerCase().match(/from\s+(orders|products|customers)/);
    ...
    const response = await axios.get(baseUrl + endpointMap[tableName], {
      headers: {
        "X-Shopify-Access-Token": accessToken,
        "Content-Type": "application/json"
      },
